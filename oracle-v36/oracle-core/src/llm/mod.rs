...
///
/// Output is always plain text now: the model is grammar-constrained to emit one
/// JSON object (a tool call or a spoken line), which the agent parses via
/// `agent::protocol`. There is no separate tool-call channel to reassemble.
...
pub struct LlmRequest {
    ...
    /// GBNF grammar constraining the output (built from the tool registry by
    /// `agent::protocol`). When set, the backend forces the model's tokens to
    /// this grammar — the reliability backbone of tool use.

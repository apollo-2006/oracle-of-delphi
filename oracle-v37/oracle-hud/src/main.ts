...
class Hud {
  ...
  private wireControls(): void {
    ...
    // Expand: leave the compact corner window and go back to the full window.
    // Core relays this to the native shell (which owns window sizing).
    const expand = document.getElementById("expand");
    expand?.addEventListener("click", () => {
      this.conn.send({ type: "expand" });
    });
    // Retract: shrink the full window back into the corner (undo a mis-click).

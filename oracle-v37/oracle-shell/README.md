...
Saying **"Delphi"** summons it too, including while it is dismissed. When
`oracle-core` hears the wake word it drops a summon flag file
(`%LOCALAPPDATA%\oracle\summon.flag` on Windows), and the shell, which checks ten
times a second, brings the window forward as a small corner panel, unless the full
window is already open and focused. "Expand" in that panel restores the full
window. The hotkey stays the summon that needs no microphone.

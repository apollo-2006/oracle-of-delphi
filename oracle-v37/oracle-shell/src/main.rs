...
fn main() -> wry::Result<()> {
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        *control_flow = ControlFlow::WaitUntil(Instant::now() + Duration::from_millis(100));
        ...
        //
        // But only when the window ISN'T already up front. If the full window is
        // open and focused, leave it exactly as it is — don't shrink it into the
        // corner mid-conversation. A hidden, minimized, or background window is
        // not focused, so the compact panel still pops in precisely those cases.
        if take_summon_flag() && !(window.is_visible() && window.is_focused()) {
            summon_compact(&window);
        }

        // "Expand" clicked in the compact window → restore the full window.
        if take_expand_flag() {
            summon_full(&window);
        }

        // "Compact" clicked in the full window → shrink back to the corner.
        if take_retract_flag() {
            summon_compact(&window);
        }
...
/// The "expand back to full" flag, written by core when the user clicks Expand
/// in the compact window. Sibling of the summon flag.
fn expand_flag_path() -> std::path::PathBuf {
    summon_flag_path().with_file_name("expand.flag")
}

/// Consume the expand flag: true once per click, deleting it so we act once.
fn take_expand_flag() -> bool {
...
/// The "shrink back to the corner" flag, written by core on a Compact click.
fn retract_flag_path() -> std::path::PathBuf {
    summon_flag_path().with_file_name("retract.flag")
}

/// Consume the retract flag: true once per click.
fn take_retract_flag() -> bool {

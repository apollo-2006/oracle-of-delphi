    ...
    fn execute(&self, turn_id: &Uuid, req: ActRequest) -> ActResponse {
        ...
        let result = match req {
            ...
            ActRequest::ReadUiTree {
                ...
                // Clamp the requested depth: deep trees explode the JSON and the
                // model's context. 12 is plenty for finding actionable controls.
                let depth = max_depth.unwrap_or(12).min(40);
                self.platform.read_ui_tree(window_id, depth).map(|els| {
                    let count = els.len();
                    json!({ "elements": els, "count": count })
                })
            }
            ActRequest::InvokeElement {
                ...
                .platform
                .invoke_element(window_id, &name, control_type.as_deref())
                .map(|el| json!({ "invoked": el })),
...
mod tests {
    ...
    #[test]
    fn read_ui_tree_is_observe_and_executes() {
        let d = daemon();
        match d.handle(env(
            1,
            ActRequest::ReadUiTree {
            ...
            ActResponse::Ok { data } => {
                let els = data["elements"].as_array().unwrap();
                assert!(els.iter().any(|e| e["name"] == "Save"));
                assert_eq!(data["count"], els.len());
            }
            other => panic!("expected ok, got {other:?}"),
        }
    }
    ...
    #[test]
    fn invoke_element_confirms_then_clicks() {
        let d = daemon();
        d.policy_mut().grant(Capability::Sensitive);
        // Sensitive (not irreversible) + granted → executes directly.
        match d.handle(env(
            1,
            ActRequest::InvokeElement {
                window_id: None,
            ...
            ActResponse::Ok { data } => assert_eq!(data["invoked"]["name"], "Save"),
            other => panic!("expected ok, got {other:?}"),
        }
    }

    #[test]
    fn invoke_element_needs_confirmation_without_grant() {
        let d = daemon();
        // No sensitive grant → parked for confirmation (Apollo), like type_text.
        match d.handle(env(
            1,
            ActRequest::InvokeElement {
                window_id: None,

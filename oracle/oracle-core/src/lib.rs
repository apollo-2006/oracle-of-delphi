...
mod integration_tests {
    ...
    async fn full_turn_runs_tool_dag_and_speaks() {
        ...
        // The three INDEPENDENT tools dispatch (gmail.search, calendar.free_slots,
        // home_assistant.light). The dependent draft references $result.1 from
        // gmail.search, which errors without Google, so its reference can't
        // resolve and it never starts — correct DAG gating. The light tool
        // succeeds locally.
        assert!(
            tools_started >= 3,
            "independent tools should dispatch (got {tools_started})"
        );
